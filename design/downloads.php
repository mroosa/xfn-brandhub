        <div class="col-full">
          <h1>Downloads</h1>
          <div class="tiles">
            <div class="tile-row" id="download-xfinity-logo">
              <h3>Xfinity Logo</h3>
              <img src="images/downloads/logo.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Xfinity Logo Materials</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-sub-brands">
              <h3>Sub-Brands</h3>
              <img src="images/downloads/sub-brands.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Xfinity Sub-Brand Materials</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-color">
              <h3>Color</h3>
              <img src="images/downloads/color.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Xfinity Color Palette</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-typography">
              <h3>Typography</h3>
              <img src="images/downloads/typography.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Typography</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-photography">
              <h3>Photography</h3>
              <img src="images/downloads/photography.png" alt="" />
              <a class="button" href="http://xfinity.com/" target="_blank"><span>Explore</span> <span class="ah">Photography</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-end-card">
              <h3>End Card</h3>
              <img src="images/downloads/endcard.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Xfinity End Card</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-powerpoint">
              <h3>Powerpoint Templates</h3>
              <img src="images/downloads/ppt.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Powerpoint Templates</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-secondary-assets">
              <h3>Secondary Assets</h3>
              <img src="images/downloads/secondary.png" alt="" />
              <a class="button" href="#"><span>Download</span> <span class="ah">Secondary Assets</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-xfinity-design-guidelines">
              <h3>Xfinity Design Guidelines</h3>
              <img src="images/downloads/brand-design-guidelines.jpg" alt="" />
              <a class="button" href="http://xfinity.com/" target="_blank"><span>Explore</span> <span class="ah">Xfinity Design Guidelines</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-messaging-guidelines">
              <h3>Messaging Guidelines</h3>
              <img src="images/downloads/messaging-guidelines.png" alt="" />
              <a class="button" href="http://xfinity.com/" target="_blank"><span>Explore</span> <span class="ah">Messaging Guidelines</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-supplemental-guidelines">
              <h3>Supplemental Guidelines</h3>
              <img src="images/downloads/supplemental-guidelines.png" alt="" />
              <a class="button" href="http://xfinity.com/" target="_blank"><span>Explore</span> <span class="ah">Supplemental Guidelines</span></a>
            </div><!-- /.download-row -->

            <div class="tile-row" id="download-olympics">
              <h3>Olympics 2020</h3>
              <img src="images/downloads/olympics.png" alt="" />
              <a class="button" href="#" target="_blank"><span>Download</span> <span class="ah">Olympics 2020 Guidelines</span></a>
            </div><!-- /.download-row -->

          </div><!-- /.downloads -->
        </div><!-- /.col-full -->
