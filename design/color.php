<div class="col-1">
  <h1>Color</h1>
  <p class="intro">Xfinity embraces a neutral color palette, which allows our products and photography to shine.</li>
  <p class="intro">To stay true to our roots, we’ll occasionally use a pop of red to show our personality and highlight important information.</li>
  <p class="intro">Action blue is used exclusively for calls-to-action in digital experiences.</li>
</div>
<div class="col-2">
  <div class="color-swatches">
    <div class="col-50 ht-100">
      <a href="#" class="color color-1 dark">
        <div class="swatch-info">
          <div>
            <h3>White</h3>
            <ul>
              <li><span>CMYK</span> 0/0/0/0</li>
              <li><span>RGB</span> 255/255/255</li>
              <li><span>HEX</span> #FFFFFF</li>
            </ul>
          </div>
        </div>
      </a><!-- color -->
    </div>
    <div class="col-50 ht-100">
      <a href="#" class="color color-2 light">
        <div class="swatch-info">
          <div>
            <h3>Black</h3>
            <ul>
              <li><span>CMYK</span> 0/0/0/100</li>
              <li><span>RGB</span> 0/0/0</li>
              <li><span>HEX</span> #000000</li>
            </ul>
          </div>
        </div>
      </a><!-- color -->
    </div>
    <div class="col-50 ht-100">
      <a href="#" class="color color-3 dark">
        <div class="swatch-info">
          <div>
            <h3>Xfinity Cool Gray</h3>
            <ul>
              <li><span>CMYK</span> 7/4/3/8</li>
              <li><span>RGB</span> 235/237/239</li>
              <li><span>HEX</span> #EBEEF0</li>
            </ul>
          </div>
        </div>
      </a><!-- color -->
    </div>
    <div class="col-50 ht-100">
      <div class="ht-66">
        <a href="#" class="color color-4 light">
          <div class="swatch-info">
            <div>
              <h3>Xfinity Dark Gray</h3>
              <ul>
                <li><span>CMYK</span> 63/56/52/27</li>
                <li><span>RGB</span> 90/90/92</li>
                <li><span>HEX</span> #5A5A5C</li>
              </ul>
            </div>
          </div>
        </a><!-- color -->
      </div>
      <div class="ht-33" style="display: flex;">
        <div class="col-50">
          <a href="#" class="color color-5 light">
            <div class="swatch-info">
              <div>
                <h3>Xfinity Accent Red</h3>
                <ul>
                  <li><span>CMYK</span> 0/100/100/10</li>
                  <li><span>RGB</span> 200/0/29</li>
                  <li><span>HEX</span> #C8001D</li>
                </ul>
              </div>
            </div>
          </a><!-- color -->
        </div>
        <div class="col-50">
          <a href="#" class="color color-6 light">
            <div class="swatch-info">
              <div>
                <h3>Xfinity Action Blue</h3>
                <ul>
                  <li><span>CMYK</span> 86/8/0/0</li>
                  <li><span>RGB</span> 43/156/216</li>
                  <li><span>HEX</span> #2B9CD8</li>
                </ul>
              </div>
            </div>
          </a><!-- color -->
        </div>
      </div>
    </div>
  </div>
</div><!-- .col-2 -->
