<div class="col-1">
  <h1>Color</h1>
  <p>Xfinity embraces a neutral color palette, which allows our products and photography to shine.</li>
  <p>To stay true to our roots, we’ll occasionally use a pop of red to show our personality and highlight important information.</li>
  <p>Action blue is used exclusively for calls-to-action in digital experiences.</p>
  <a class="button" href="#downloads"><span>Download Color Palette</span></a>
</div>
<div class="col-2">
  <div class="tabs-wrapper">
    <div class="tabs">

      <div class="tab" id="color-palette" data-ttl="Color Palette">
        <div class="color-swatches">
          <div class="col-50 ht-100">
            <a href="#color-white" class="color color-1 dark ignore-click" title="White"></a>
          </div>
          <div class="col-50 ht-100">
            <a href="#color-black" class="color color-2 light ignore-click" title="Black"></a>
          </div>
          <div class="col-50 ht-100">
            <a href="#color-cool-gray" class="color color-3 dark ignore-click" title="Xfinity Cool Gray"></a>
          </div>
          <div class="col-50 ht-100">
            <div class="ht-66">
              <a href="#color-dark-gray" class="color color-4 light ignore-click" title="Xfinity Dark Gray"></a>
            </div>
            <div class="ht-33" style="display: flex;">
              <div class="col-50">
                <a href="#color-accent-red" class="color color-5 light ignore-click" title="Xfinity Accent Red"></a>
              </div>
              <div class="col-50">
                <a href="#color-action-blue" class="color color-6 light ignore-click" title="Xfinity Action Blue"></a>
              </div>
            </div>
          </div>
        </div><!-- /.color-swatches -->
        <div class="color-swatch-info">
          <div id="color-white" class="swatch-info dark" title="">
            <div>
              <h3>White</h3>
              <ul>
                <li><span>CMYK</span> 0/0/0/0</li>
                <li><span>RGB</span> 255/255/255</li>
                <li><span>HEX</span> #FFFFFF</li>
              </ul>
            </div>
          </div>
          <div id="color-black" class="swatch-info light" title="">
            <div>
              <h3>Black</h3>
              <ul>
                <li><span>CMYK</span> 0/0/0/100</li>
                <li><span>RGB</span> 0/0/0</li>
                <li><span>HEX</span> #000000</li>
              </ul>
            </div>
          </div>
          <div id="color-cool-gray" class="swatch-info dark" title="">
            <div>
              <h3>Xfinity Cool Gray</h3>
              <ul>
                <li><span>CMYK</span> 7/4/3/8</li>
                <li><span>RGB</span> 235/237/239</li>
                <li><span>HEX</span> #EBEEF0</li>
              </ul>
            </div>
          </div>
          <div id="color-dark-gray" class="swatch-info light" title="">
            <div>
              <h3>Xfinity Dark Gray</h3>
              <ul>
                <li><span>CMYK</span> 63/56/52/27</li>
                <li><span>RGB</span> 90/90/92</li>
                <li><span>HEX</span> #5A5A5C</li>
              </ul>
            </div>
          </div>
          <div id="color-accent-red" class="swatch-info light" title="">
            <div>
              <h3>Xfinity Accent Red</h3>
              <ul>
                <li><span>CMYK</span> 0/100/100/10</li>
                <li><span>RGB</span> 200/0/29</li>
                <li><span>HEX</span> #C8001D</li>
              </ul>
            </div>
          </div>
          <div id="color-action-blue" class="swatch-info light" title="">
            <div>
              <h3>Xfinity Action Blue</h3>
              <ul>
                <li><span>CMYK</span> 86/8/0/0</li>
                <li><span>RGB</span> 43/156/216</li>
                <li><span>HEX</span> #2B9CD8</li>
              </ul>
            </div>
          </div>
        </div><!-- /.color-swatch-info -->
      </div>

      <!-- tab -->
      <!-- <div class="tab" id="color-in-use" data-ttl="Color In Use">
        <div class="tab-inner">
          <h3>Color In Use</h3>
          <ul class="optionWrap" title="">
            <li class="img width-75"><img class="rounded" src="<?php print $dir; ?>images/color/01.png" alt="" /></li>
            <li class="img width-50"><img class="border rounded" src="<?php print $dir; ?>images/color/02.png" alt="" /></li>
            <li class="img width-35"><img class="border rounded" src="<?php print $dir; ?>images/color/03.png" alt="" /></li>
          </ul>
        </div>
      </div> -->

      <!-- tab -->
      <div class="tab" data-ttl="Misuses" data-bg="white" id="color-misuses">
        <div class="tab-inner">
          <h3>Color Misuses</h3>
          <ul class="optionWrap">
            <li title="Do not overuse red or re-color icons." class="img width-55"><img style="max-height: 425px;" class="rounded" src="<?php print $dir; ?>images/color/misuse-1.png" alt="" /></li>
            <li title="Do not use red or blue in headlines or subheads. Do not use the limited use red logo in communications." class="img"><img style="max-height: 350px;" class="border rounded" src="<?php print $dir; ?>images/color/misuse-0.png" alt="" /></li>
            <li title="Do not use red in full bleed or as a background color." class="img width-55"><img style="max-height: 425px;" class="border rounded" src="<?php print $dir; ?>images/color/misuse-2.png" alt="" /></li>
          </ul>
        </div>
      </div>

    </div>
  </div>
</div><!-- .col-2 -->
